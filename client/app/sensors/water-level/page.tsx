...
import WaterLevelCurrent from './WaterLevelCurrent';
import WaterLevel from "./WaterLevelGraph";
    ...
    return (
        <div className="space-y-6">
            <div className='flex justify-between'>
                <div>
                    <h1 className="text-2xl font-semibold">Water Level Details</h1>
                    <p className="text-sm text-muted-foreground">
                        Latest water level reading and sensor info
                    </p>
                </div>
                <Button variant="outline" size="sm">
                    <Link href="/sensors/water-level/settings" className="flex items-center gap-2">
                        <Settings />
                        Settings
                    </Link>
                </Button>
            ...
            {/* Current water level + graph */}
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <WaterLevelCurrent sensor={sensor} loading={loading} />

                {/* Graph and table */}
                <WaterLevel readings={readings} sensor={sensor} loading={loading} />
            </div>

            <WaterLevelTable sensor={sensor} readings={readings} loading={loading} />
        </div>
    );
}
