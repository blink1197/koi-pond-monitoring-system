...
import TurbidityCurrent from "./TurbidityCurrent";
import TurbidityGraph from "./TurbidityGraph";
import TurbidityTable from "./TurbidityTable";
    ...
    return (
        <div className="space-y-6">
            <div className='flex justify-between'>
                <div>
                    <h1 className="text-2xl font-semibold">Turbidity Details</h1>
                    <p className="text-sm text-muted-foreground">
                        Latest turbidity reading and sensor info
                    </p>
                </div>
                <Button variant="outline" size="sm">
                    <Link href="/sensors/turbidity/settings" className="flex items-center gap-2">
                        <Settings />
                        Settings
                    </Link>
                </Button>
            ...
            {/* Current turbidity + graph */}
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <TurbidityCurrent sensor={sensor} loading={loading} />

                {/* Graph and table */}
                <TurbidityGraph readings={readings} sensor={sensor} loading={loading} />
            </div>

            <TurbidityTable sensor={sensor} readings={readings} loading={loading} />
        </div>
    );
}
